...
// Railwayはエフェメラルストレージ。/tmp を使う（再起動でリセットされる）
const UPLOAD_DIR = process.env.UPLOAD_DIR || path.join(require('os').tmpdir(), 'etofile-uploads');
...
app.post('/upload', (req, res) => {
  upload.array('files')(req, res, (err) => {
    ...
    try {
      ...
      setTimeout(() => deleteShare(shareId), expireHours * 3600 * 1000);
...
function deleteShare(shareId) {
  const share = fileStore.get(shareId);
  if (!share) return;
